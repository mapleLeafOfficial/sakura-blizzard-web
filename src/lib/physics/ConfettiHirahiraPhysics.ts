...
//      in all directions, then smoothly transitioning to a gentle hirahira
//      (flutter) falling motion via linear interpolation.
// (ja) 全方位に飛び散った後、線形補間によってなめらかにひらひら落下へ
//      移行する物理演算クラスです。
///
// 1:1 port of: lib/src/physics/confetti_hirahira_physics.dart
export class ConfettiHirahiraPhysics extends Sp3dPhysics {
  /// 爆発フェーズの速度（内部状態）
  private _vx: number;
  private _vy: number;
